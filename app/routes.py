...
@login_required
def index():
    if current_user.role == 'internal': return redirect(url_for('main.internal_dashboard'))
    elif current_user.role == 'partner': return redirect(url_for('main.portal_dashboard'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            flash('无效用户名或密码', 'danger'); return redirect(url_for('main.login'))
...
def internal_dashboard():
    # 1. 统计
    rev_stats = db.session.query(IpAsset.name, IpAsset.current_revenue).order_by(desc(IpAsset.current_revenue)).limit(5).all()
    rev_x = [r[0] for r in rev_stats]; rev_y = [float(r[1]) for r in rev_stats]

    click_stats = db.session.query(IpAsset.name, func.count(IpAnalytics.id)).join(IpAnalytics).group_by(IpAsset.name).order_by(func.count(IpAnalytics.id).desc()).limit(5).all()
    click_x = [c[0] for c in click_stats]; click_y = [c[1] for c in click_stats]
    ...
    ct_q_type = request.args.get('ct_type', '') # 新增参数
    ...
    if ct_q_type: ct_query = ct_query.filter(Contract.license_type == ct_q_type) # 新增筛选
    ...
    return render_template('internal_dashboard.html', title='内控台账',
                           ...
                           search_ct_partner=ct_q_partner, search_ct_ip=ct_q_ip, search_ct_type=ct_q_type,
...
def add_ip():
    ...
    if form.validate_on_submit():
        ...
        new_ip = IpAsset(
            name=form.name.data, tags=form.tags.data, description=form.description.data, image_url=img,
...
def delete_ip(ip_id):
    ip = IpAsset.query.get_or_404(ip_id)
    if ip.contracts: flash('无法删除：请先删除关联的合同', 'danger')
    else: db.session.delete(ip); db.session.commit(); flash('IP 已删除', 'success')
    return redirect(url_for('main.internal_dashboard'))
...
def api_get_database_info():
    try:
        ...
        report = ["【IP 资产全量数据】"]
        for ip in ips:
            report.append(f"名称: {ip.name} | 级别: {ip.value_level} | 收益: {ip.current_revenue}万")
            report.append(f"权属: {ip.ownership} | 商标: {ip.trademark_info}")
            report.append(f"状态: {ip.internal_status} | 授权期: {ip.license_period}")
            report.append("---")
        contracts = Contract.query.all()
        report.append("\n【合同数据】")
        for c in contracts:
            report.append(f"相对方: {c.partner_name} | IP: {c.ip_asset.name}")
            report.append(f"类型: {c.license_type} | 费用: {c.fee_standard}")
            report.append("---")
...
def api_generate_contract_doc():
    ...
    try:
        doc = Document()
        doc.add_heading('合同初稿', 0)
        doc.add_paragraph(f"甲方：星核文化\n乙方：{data.get('partner_name')}\n金额：{data.get('money')}")
        ...
        return jsonify({"status": "success", "download_url": url_for('static', filename=f'generated_docs/{fn}', _external=True)})
    except Exception as e: return jsonify({"status": "error", "message": str(e)}), 500